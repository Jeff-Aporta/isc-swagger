/**
 * hojas.test.mjs — el parche que quita el flicker de los shadow roots del kit.
 *
...
test('el primer componente conserva su <link>: nunca se queda sin estilos', async () => {
  ...
  assert.ok(shadow.querySelector('link[rel="stylesheet"]'), 'el primer <link> debe pasar tal cual');
...
test('el kit prepende dos hojas de golpe y las dos se adoptan', () => {
  ...
  assert.equal(primero.querySelectorAll('link[rel="stylesheet"]').length, 1);
  assert.equal(primero.querySelector('link').href, scrollbars);
...
test('los sw-* comparten el caché: una sola descarga por hoja', async () => {
  ...
  assert.equal(segundo.querySelector('link[rel="stylesheet"]'), null);
